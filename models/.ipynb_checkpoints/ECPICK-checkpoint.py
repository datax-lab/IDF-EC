...
from utils.utils import SeqEncoder, Flatten, NumpyPairDataset
...
class ECPICK(nn.Module):
    ...
    def loss(self, global_final_output, local_final_output, y, weights=None):
        # Compute Global Loss
        if weights is not None:
            global_loss = F.binary_cross_entropy(global_final_output, y, weight=weights, reduction="sum")
        else:
            global_loss = F.binary_cross_entropy(global_final_output, y, reduction="sum")

        # Compute Local Loss
        if weights is not None:
            local_loss = F.binary_cross_entropy(local_final_output, y, weight=weights, reduction="sum")
        else:
            local_loss = F.binary_cross_entropy(local_final_output, y, reduction="sum")
...
class PredictionModel(nn.Module):
    def __init__(self, output_classes, cuda_support, cuda_device, dropout_rate, relu_size, beta, model_path):
        ...
        # region [ Load HD2W Model ]
        self.model = ECPICK(output_classes, cuda_support=cuda_support, cuda_device=cuda_device,
                            dropout_rate=dropout_rate, relu_size=relu_size, beta=beta)
...
class InterpretationModel(nn.Module):
    ...
    def __init__(self, output_classes, cuda_support, cuda_device, dropout_rate, relu_size, beta, model_path):
        ...
        # region [ Hook Placeholder for interpretation ]
        # placeholder for the interpretation
        self.cnn_1_forward_result = None
        self.cnn_2_forward_result = None
        self.cnn_3_forward_result = None

        self.max_pool_1_backward_result = None
        self.max_pool_2_backward_result = None
        self.max_pool_3_backward_result = None
        # endregion
    ...
    def register_hook(self):
        self.model.cnn1[0].register_forward_hook(self.cnn_1_forward_hook)
        self.model.cnn2[0].register_forward_hook(self.cnn_2_forward_hook)
        self.model.cnn3[0].register_forward_hook(self.cnn_3_forward_hook)

        self.model.cnn1[2].register_backward_hook(self.max_pool_1_backward_hook)
        self.model.cnn2[2].register_backward_hook(self.max_pool_2_backward_hook)
        self.model.cnn3[2].register_backward_hook(self.max_pool_3_backward_hook)
